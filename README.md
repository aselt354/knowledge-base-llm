...
Каждый запуск сохраняет два файла в папку `results/` с единой временной меткой:

| Файл | Содержимое |
|------|------------|
| `results_YYYYMMDD_HHMMSS.json` | Точность по шагам и категориям, все предсказания с reasoning |
| `text_log_YYYYMMDD_HHMMSS.txt` | Полный вывод запуска — ответы LLM, пошаговые результаты, итоговая статистика |
...
```
автоматически:
- берётся последний `results_*.json
- строятся 2 графика
